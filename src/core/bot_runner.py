...
class BotRunner:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.web_engine, self.gathering, self.healing, self.steps, self.combat, self.captcha, self.quest_automation = (
                systems
            )
            return True
    ...
    async def run_cycle(self) -> dict[str, bool]:
        ...
        try:
            # FIRST: Check if page context was destroyed (navigation crash)
            if await self.web_engine.is_context_destroyed():
                logger.error("🚨 Page navigation detected - bot context destroyed!")
                logger.error("🛑 Stopping bot to prevent errors. Please restart when ready.")
                self.running = False
                results["context_destroyed"] = True
                await self.web_engine.handle_context_destruction()
                return results
    ...
    async def reset_state(self):
        """Reset all bot systems to initial state when bot is stopped and restarted"""
        ...
        # Reset captcha system state
        if self.captcha and hasattr(self.captcha, "reset_state"):
            await self.captcha.reset_state()

        # Reset steps system state
        if self.steps and hasattr(self.steps, "reset_state"):
            await self.steps.reset_state()

        # Reset combat system state
        if self.combat and hasattr(self.combat, "reset_state"):
            await self.combat.reset_state()

        logger.success("✅ Bot state reset complete - ready for fresh start")
...
async def initialize_systems(config: "BotConfig") -> tuple[Any, ...] | None:
    ...
    systems = [
        ...
        ("Combat", combat),
        ("Captcha", captcha),
        ("Quest Automation", quest_automation),
    ]
    ...
    return web_engine, gathering, healing, steps, combat, captcha, quest_automation
...
async def check_and_handle_quests(quest_automation, config) -> bool:
    """Check and handle quest opportunities"""
    try:
        # Verifica se quest automation está habilitado na config
        if not config.get("quests_enabled", False):
            return False

        # Verifica quest points disponíveis
        current_points, max_points = await quest_automation.get_quest_points()
        if current_points <= 0:
            logger.debug("⚡ No quest points available")
            return False

        logger.info(f"🎯 Quest points available: {current_points}/{max_points}")

        # Executa ciclo de quests
        max_quests_per_cycle = config.get("max_quests_per_cycle", 3)
        results = await quest_automation.execute_quests_cycle(max_quests_per_cycle)

        if results["quests_successful"] > 0:
            logger.success(f"✅ Completed {results['quests_successful']} quests")
            return True
        else:
            logger.debug("⚡ No quests completed this cycle")
            return False

    except Exception as e:
        logger.error(f"❌ Error handling quests: {e}")
        return False
